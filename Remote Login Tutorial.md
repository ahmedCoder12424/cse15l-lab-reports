...
The `cd` command changes directory and two commands show run without error. The `ls` command lists files in a certain directory and show different files for each of three `ls` commands shown. The `cp` command copies the file `hello.txt` into the current directory. Lastly, the `cat` shows the contents of the `hello.txt` file.
